...
export async function autoGenerateDailyDrafts() {
  ...
  for (const user of users) {
    ...
    const question = activeQuestions[randomIndex];
    ...
    try {
      ...
      for (const platform of platforms) {
        const generatedCopy = await generateText({
          platform,
          tone: "professional",
          topic: `Question to address: ${question.text}. Share insightful professional tips related to this.`,
          goal: "educate",
        });
        draftsMap[platform] = generatedCopy;
      }

      const imgPrompt = `A high quality, professional, conceptual modern graphic representing: ${question.text}`;
      const imageBuffer = await generateImage({ prompt: imgPrompt });

      const upload = await uploadImageBuffer(imageBuffer, "postforge/ai-generated");
      ...
      const post = await Post.create({
        ...
        aiPrompt: `Daily Auto-Generated Draft from Question: ${question.text}`,
        platforms,
